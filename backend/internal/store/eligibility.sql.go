...
const getDonorEligibilityFacts = `-- name: GetDonorEligibilityFacts :one
WITH completed AS (
...
of_procedure AS (
    ...
    FROM completed
    WHERE procedure = $2::donation_procedure
),
ever AS (
...
type GetDonorEligibilityFactsParams struct {
	DonorID   int64
	Procedure DonationProcedure
}

type GetDonorEligibilityFactsRow struct {
	DonorID             int64
	DateOfBirth         pgtype.Date
	Gender              Gender
	AccountActive       bool
	FirstTime           bool
	LastDonationAt      pgtype.Timestamptz
	DonationsLast12m    int32
...
func (q *Queries) GetDonorEligibilityFacts(ctx context.Context, arg GetDonorEligibilityFactsParams) (GetDonorEligibilityFactsRow, error) {
	row := q.db.QueryRow(ctx, getDonorEligibilityFacts, arg.DonorID, arg.Procedure)
	var i GetDonorEligibilityFactsRow
	err := row.Scan(
		&i.DonorID,
		&i.DateOfBirth,
		&i.Gender,
...
const getDonorEligibilityView = `-- name: GetDonorEligibilityView :one
...
type GetDonorEligibilityViewRow struct {
	DonorID             int64
	AgeYears            int32
...
func (q *Queries) GetDonorEligibilityView(ctx context.Context, donorID int64) (GetDonorEligibilityViewRow, error) {
	row := q.db.QueryRow(ctx, getDonorEligibilityView, donorID)
	var i GetDonorEligibilityViewRow
	err := row.Scan(
		&i.DonorID,
		&i.AgeYears,
		&i.LastDonatedAt,
